...
int axk_send_notify(uint8_t *buf, uint8_t len)
{
    ...
    {
//        memcpy(pNoti.value, UploadStatusValue, sizeof(UploadStatusValue)/sizeof(UploadStatusValue[0]));
			return -1;
    }

    set_Bleuart_Notify();
    bleuart_Notify(ConnHandle, &pNoti, 0);
    clear_Bleuart_Notify();

    
    for (uint16_t i = 0; i < len; i++)
    {
        AT_LOG("0X%02X ", pNoti.value[i]);
    }
    AT_LOG("\r\n");
