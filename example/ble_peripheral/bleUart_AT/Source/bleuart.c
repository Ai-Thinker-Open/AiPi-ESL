...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "ESL01";
...
extern int EPD_cmd_process(uint8_t *data, uint32_t size);
...
//			AT_LOG("pev->data:%s, size:%d\r\n", pev->data, pev->param);
			ret = EPD_cmd_process(pev->data, pev->param);
			AT_LOG("EPD_cmd_process ret:%d\r\n", ret);
        break;
...
void bleuart_Init( uint8 task_id )
{
    ...
    //update advertise data.
		snprintf((char *)&m_at_ctx.mod_name[4], 5, "%02X%02X", m_at_ctx.mac_addr[4], m_at_ctx.mac_addr[5]);
...
//        advInt = m_at_ctx.adv_int;
    ...
    if(m_at_ctx.auto_slp_time == 0) // Use default value(20s)in case it's still not set before.
        osal_start_timerEx( bleuart_TaskID, BUP_OSAL_EVT_AT_AUTO_SLEEP, 10 * 1000 );
    else
...
void reset_Transmit_timer(uint32_t Ms)
{
	osal_stop_timerEx(bleuart_TaskID, BUP_OSAL_EVT_TRANSMIT_TIMEOUT);
	osal_start_timerEx(bleuart_TaskID, BUP_OSAL_EVT_TRANSMIT_TIMEOUT, Ms);
}
void stop_Transmit_timer(void)
{
	osal_stop_timerEx(bleuart_TaskID, BUP_OSAL_EVT_TRANSMIT_TIMEOUT);
}
extern void TransmitTimeOut(void);
/*********************************************************************
...
//    case BUP_OSAL_EVT_UART_DATA_RX:  // for DMA only
    if ( events & BUP_OSAL_EVT_UART_DATA_RX )
    {
		...
		if ( events & BUP_OSAL_EVT_TRANSMIT_TIMEOUT )
    {
        AT_LOG("BUP_OSAL_EVT_TRANSMIT_TIMEOUT\n");
				TransmitTimeOut();

        return ( events ^ BUP_OSAL_EVT_TRANSMIT_TIMEOUT );
    }
